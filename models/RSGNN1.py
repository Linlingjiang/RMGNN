...
class RSGNN:
    ...
    def train_adj(self, epoch, features, edge_index, labels, idx_train, idx_val):
        ...
        rec_loss,separateloss,compactloss = self.estimator(edge_index,features)
        ...
        total_loss = loss_gcn + args.lamb * separateloss + args.lamb1 * compactloss
...
class EstimateAdj(nn.Module):
    ...
    def __init__(self, edge_index, features, args ,device='cuda'):
        ...
        if args.estimator=='MLP':
            ...
            self.\
                estimator = GCN(features.shape[1], args.mlp_hidden, args.mlp_hidden,dropout=0.0,device=device)
        self.device = device
        self.args = args
        ...
        self.query_projection = nn.Linear(features.shape[1],args.mlp_hidden)
        self.separate_loss = nn.TripletMarginLoss(margin=1.0)
        self.compact_loss = nn.MSELoss()

    def construct_memory(self):
        memory_dict = nn.ParameterDict()
        memory_dict['Memory'] = nn.Parameter(torch.randn(20, 64), requires_grad=True)     # (M, d)
        memory_dict['Wq'] = nn.Parameter(torch.randn(64, 64), requires_grad=True)    # project to query
        # memory_dict['We1'] = nn.Parameter(torch.randn(self.num_nodes, self.rnn_units), requires_grad=True) # project memory to embedding
        # memory_dict['We2'] = nn.Parameter(torch.randn(self.num_nodes, self.rnn_units), requires_grad=True) # project memory to embedding
    ...
    def query_memory(self, h_t: torch.Tensor):
        ...
        # 获取最大的一个元素及其索引
        _, max_ind = torch.topk(att_score, k=1, dim=-1, largest=True)
        # 获取最小的一个元素及其索引
        _, min_ind = torch.topk(att_score, k=1, dim=-1, largest=False)
        # _, ind = torch.topk(att_score, k=2, dim=-1)
        pos = self.memory['Memory'][max_ind] # B, N, d
        neg = self.memory['Memory'][min_ind] # B, N, d
    ...
    def forward(self, edge_index, features):
        ...
        representations, query, pos, neg = self.query_memory(features)
